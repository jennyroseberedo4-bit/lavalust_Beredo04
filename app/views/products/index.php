...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        body { min-height: 100vh; margin: 0; padding: 42px 24px; color: var(--ink); font-family: 'Manrope', sans-serif; background: linear-gradient(rgba(255,255,255,.38) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,.38) 1px, transparent 1px), radial-gradient(circle at 90% 8%, rgba(192,68,163,.2), transparent 25%), var(--lavender); background-size: 32px 32px, 32px 32px, auto, auto; }
        main { width: min(1160px, 100%); margin: auto; }
        .topbar, .actions, .product-card { border: 1px solid var(--line); background: rgba(255,250,255,.9); }
        .topbar { display: flex; align-items: end; justify-content: space-between; gap: 20px; padding: 28px 30px; border-radius: 16px 16px 0 0; }
        .eyebrow { margin: 0 0 12px; color: var(--magenta); font: 500 .72rem 'DM Mono', monospace; letter-spacing: .12em; text-transform: uppercase; }
        ...
        a, button { border: 0; border-radius: 7px; padding: 10px 14px; color: #fff; background: var(--violet); font: 700 .8rem 'Manrope', sans-serif; text-decoration: none; cursor: pointer; }
        a:hover, button:hover { background: var(--magenta); }
        .logout { color: var(--violet); background: transparent; border: 1px solid var(--line); }
        .product-card { overflow-x: auto; border-top: 0; border-radius: 0 0 16px 16px; box-shadow: 10px 10px 0 #c7b1f4; }
        table { width: 100%; min-width: 820px; border-collapse: collapse; }
        th, td { padding: 17px 20px; border-bottom: 1px solid var(--line); text-align: left; }
        th { color: var(--muted); background: #f4edff; font: 500 .68rem 'DM Mono', monospace; letter-spacing: .1em; text-transform: uppercase; }
        td { font-size: .9rem; vertical-align: top; }
        td:first-child { color: var(--magenta); font-family: 'DM Mono', monospace; }
        td:nth-child(3) { color: var(--violet); font-weight: 700; }
        .description { max-width: 280px; color: var(--muted); }
        .row-actions { display: flex; gap: 8px; }
        .edit { color: var(--violet); background: #ede3ff; }
        .delete { color: #a1266f; background: #f8dff0; }
