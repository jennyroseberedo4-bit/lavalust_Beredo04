...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        body { min-height: 100vh; margin: 0; display: grid; place-items: center; padding: 24px; color: var(--ink); font-family: 'Manrope', sans-serif; background: radial-gradient(circle at 10% 15%, rgba(217,29,61,.18), transparent 28%), linear-gradient(135deg, #ffe7ec, #fff6f8); }
        .card { width: min(520px, 100%); padding: 38px; border: 1px solid var(--line); border-top: 4px solid var(--red); border-radius: 18px; background: rgba(255,247,248,.94); box-shadow: 12px 12px 0 rgba(217,29,61,.18); }
        ...
        input:focus { outline: 3px solid rgba(217,29,61,.15); border-color: var(--red); }
        button { width: 100%; margin-top: 26px; padding: 14px; border: 0; border-radius: 8px; color: #fff; background: var(--red); font: 700 .85rem 'Manrope', sans-serif; cursor: pointer; }
        button:hover { background: var(--pink); }
        .error { margin: 0 0 18px; padding: 12px; border-radius: 8px; color: #8e1030; background: var(--rose); font-size: .86rem; }
